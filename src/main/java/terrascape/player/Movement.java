...
public final class Movement {
    ...
    private void handleInputMovementStateChange(Vector3f position) {
        ...
        if (window.isKeyPressed(CRAWL_BUTTON)) {
            ...
            if (!collidesWithMaterial(position.x, position.y + 1.25f, position.z, WALKING)) {
                ...
                movementState = WALKING;
            } else if (!collidesWithMaterial(position.x, position.y + 1.0f, position.z, CROUCHING)) {
    ...
    private void handleNonCollisionStopping(float y, Vector3f oldPosition, Vector3f position) {
        ...
        if ((movementState == CROUCHING || movementState == CRAWLING) && isGrounded && y <= 0.0f && collidesWithMaterial(oldPosition.x, position.y - 0.0625f, oldPosition.z, movementState)) {
